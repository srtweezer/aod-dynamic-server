...
spcm_drv.h                                     (c) Spectrum GmbH , 08/2005

**************************************************************************

Interface of the spectrum driver for all M2I and newer cards. 
...
// Sets a dedicated software register using a pointer with a specific length. Return value is an error code
SPCM_IMPORT uint32 _stdcall spcm_dwSetParam_ptr (
    drv_handle  hDevice,                // handle to an already opened device
    int32       lRegister,              // software register to be modified
    void*       pvValue,                // pointer for the return value
    uint64      qwLen);                 // length of content of pvValue

//*************************************************************************

// Sets a software register using 1 x double value. Return value is an error code
SPCM_IMPORT uint32 _stdcall spcm_dwSetParam_d64 (   
    drv_handle  hDevice,                // handle to an already opened device
    int32       lRegister,              // software register to be modified
...
// Reads a dedicated software register using a pointer with a specific length. Return value is an error code
SPCM_IMPORT uint32 _stdcall spcm_dwGetParam_ptr (
    drv_handle  hDevice,                // handle to an already opened device
    int32       lRegister,              // software register to be read out
    void*       pvValue,                // pointer for the return value
    uint64      qwLen);                 // length of the buffer behind pvValue

//*************************************************************************

// Reads out a software register using 1 x double value. Return value is an error code
SPCM_IMPORT uint32 _stdcall spcm_dwGetParam_d64 (   
    drv_handle  hDevice,                // handle to an already opened device
    int32       lRegister,              // software register to be read out
    double*     pdValue);               // pointer for the return value
...
SPCM_IMPORT uint32 _stdcall spcm_dwGetErrorInfo_i32 (
    drv_handle  hDevice,                           // handle to an already opened device (can be NULL to read errors that occurred during spcm_hOpen())
    ...
    char        pszErrorTextBuffer[ERRORTEXTLEN]); // text buffer for text error (can be NULL if not of interest)

SPCM_IMPORT uint32 _stdcall spcm_dwGetErrorInfo_i64 (
    drv_handle  hDevice,                           // handle to an already opened device (can be NULL to read errors that occurred during spcm_hOpen())
    uint32*     pdwErrorReg,                       // address of the error register (can be NULL if not of interest)
    int64 *     pllErrorValue,                     // address of the error value (can be NULL if not of interest)
    char        pszErrorTextBuffer[ERRORTEXTLEN]); // text buffer for text error (can be NULL if not of interest)

SPCM_IMPORT uint32 _stdcall spcm_dwGetErrorInfo_d64 (
    drv_handle  hDevice,                           // handle to an already opened device (can be NULL to read errors that occurred during spcm_hOpen())
    uint32*     pdwErrorReg,                       // address of the error register (can be NULL if not of interest)
